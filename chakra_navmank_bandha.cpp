...
// Generates a generic chakra bandha of size n
int** generate_n_sized_chakra_bandha(const int n) {
    // this array is to be filled with a chakra bandha of size n
    ...
    while (true) {
        ...
        chakra_bandha[row][collumn] = value;
...
int* bandha(int** chakra, int** bandha, const int n) {
...
// returns transposed text when a chakra is applied with chakra bandha of size 27 (the ubiquitous chakra bandha)
int* chakra_bandha_27(int** chakra) {
  ...
  {352, 381, 410, 439, 468, 497, 526, 555, 584, 613, 642, 671, 700, 729, 2, 31, 60, 89, 118, 147, 176, 205, 234, 263, 292, 321, 350}
    ...
    int** chakra_bandha = new int*[27];
    for (int i = 0; i < 27; i++) {
        chakra_bandha[i] = new int[27];
        for (int j = 0; j < 27; j++) {
            chakra_bandha[i][j] = temp_chakra_bandha[i][j];
        }
    }
    return bandha(chakra, chakra_bandha, 27);
}

// returns transposed text when a 27*27 chakra is applied with navmank bandha
// the 27*27 chakra is divded into 9 subchakras of size 9*0

int* navmank_bandha(int** chakra, int* order) {
  ...
  {37, 48, 59, 70, 81, 2, 13, 24, 35}
    ...
    int* transposed = new int[27 * 27];    
    for (int sub_chakraN = 0; sub_chakraN < 9; sub_chakraN++) {
        // get how much from the left and top the subchakra is
        const int from_left = (sub_chakraN % 3) * 9;
        const int from_top = (sub_chakraN / 3)* 9;
        const int gap_to_leave_in_transposed = (order[sub_chakraN]-1) * 9 * 9;
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                transposed[navmank_bandha[i][j] - 1 + gap_to_leave_in_transposed] = chakra[i + from_top][j + from_left];
            }
        }
}
    // delete navmank_bandha
    for (int i = 0; i < 9; i++) {
        delete[] navmank_bandha[i];
    }
    delete[] navmank_bandha;
    return transposed;
}
...
int main() {
    // the size of our chakra and bandha
    const int n = 3;
    // initialize chakra


    int initial_values[n][n] = {
        {1, 12, 13},
        {60, 5, 26},
        {7, 18, 39}
    };
    ...
    for (int i = 0; i < n; i++) {
         chakra[i] = new int[n];
        for (int j = 0; j < n; j++) {
            chakra[i][j] = initial_values[i][j];
        }
    }
    // we have initialized the chakra
    // generate a chakra bandha of size n
    int** my_chakra_bandha = generate_n_sized_chakra_bandha(n);
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
    ...
    // Transpose chakra using the bandha
    int* transposed = bandha(chakra, my_chakra_bandha, n);
    for (int i = 0; i < n * n; i++) {
        cout << transposed[i] << " ";
    }
    cout << endl;
