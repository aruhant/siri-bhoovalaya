...
export class BrahmiCustomEditDistance implements DistanceCalculator {
  ...
  isDistanceLowerThan(word1: Word, word2: Word, distance: number): boolean {
    // return the same value as this.getDistance(word1, word2) < distance;
    // but with a more efficient implementation. We need to compute the
    // distance only if it is lower than the given distance.
    // This is useful when the distance is very high and we can avoid
    // computing the distance if it is not necessary.
    // We abort the computation as soon as we know the distance is higher.
    const m = word1.length();
    const n = word2.length();
    if (Math.abs(m - n) > distance) {
      return false;
    }
    ...
    for (let j = 0; j <= n; j++) {
      let cost = 0;
      for (let k = 0; k < j; k++) {
        cost += this.insert_map.get(word2.unitAt(k).get()) ?? 1;
      }
      dp[0][j] = cost;
      if (cost > distance) {
        return false;
    ...
    for (let i = 0; i <= m; i++) {
      let cost = 0;
      for (let k = 0; k < i; k++) {
        cost += this.delete_map.get(word1.unitAt(k).get()) ?? 1;
      }
      dp[i][0] = cost;
      if (cost > distance) {
        return false;
